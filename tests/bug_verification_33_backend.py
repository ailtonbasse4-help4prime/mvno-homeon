#!/usr/bin/env python3
"""Focused backend verification for iteration 33 Asaas payment-history block simulation bug.
...
OUT_FILE = ROOT / "test_reports" / "bug33_backend_results.json"
...
def create_case(db, tag, label, cobrancas, asaas_customer_id=None, whitelist=False):
    ...
    if whitelist:
        db.automacao_bloqueio_whitelist.insert_one({
            "cliente_id": cid_s,
            "motivo": "QA BUG33 whitelist regression",
            "added_by": "qa",
            "added_by_name": "QA",
            "added_at": now,
            "qa_tag": tag,
